/*
 * $Id: ProcessInvocation.java 1036 2010-11-24 08:47:10Z jcalleja $
 * --------------------------------------------------------------------------------------
...
/**
 * ProcessInvocation requires a MuleMessage with a payload which contains a
 * ProcessRequest[] and a String (i.e. the sessionId) as the first and second
 * parameter respectively.
 */
public class ProcessInvocation extends AbstractInvocation
{

    public ProcessInvocation(GenericKeyedObjectPool soapBindingPool, Object[] invocationParameters)
    {
    ...
    public Object invoke() throws Exception
    {
        ...
        ProcessRequest[] actions = (ProcessRequest[]) invocationParameters[0];
        ...
        try
        {
            ...
            {
                logger.debug("Before executing process: SessionId = " + sessionId);
            }

            result = sfdc.process(actions);

            if (logger.isDebugEnabled())
            {
                logger.debug("After executing process: SessionId = " + sessionId);
            }
        }
        finally
        {
