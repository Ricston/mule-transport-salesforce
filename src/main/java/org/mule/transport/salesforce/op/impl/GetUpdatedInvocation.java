/*
 * $Id: GetUpdatedInvocation.java 1036 2010-11-24 08:47:10Z jcalleja $
 * --------------------------------------------------------------------------------------
...
/**
 * GetUpdatedInvocation requires a MuleMessage with a payload which contains a
 * String, Calendar, Calendar, and a String (i.e. the sessionId) as the first,
 * second, third, and fourth parameter respectively.
 */
public class GetUpdatedInvocation extends AbstractInvocation
{

    public GetUpdatedInvocation(GenericKeyedObjectPool soapBindingPool, Object[] invocationParameters)
    {
    ...
    public Object invoke() throws Exception
    {
        ...
        try
        {
            ...
            {
                logger.debug("Before executing getUpdated: sObjectType = " + sObjectType + ", startDate = "
                             + PrintUtil.printCalendar(startDate) + ", endDate = "
                             + PrintUtil.printCalendar(endDate) + ", SessionId = " + sessionId);
            }

            result = sfdc.getUpdated(sObjectType, startDate, endDate);

            if (logger.isDebugEnabled())
            {
                logger.debug("After executing getUpdated: sObjectType = " + sObjectType + ", SessionId = "
                             + sessionId);
            }
        }
        finally
